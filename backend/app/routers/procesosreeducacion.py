...
def crear_proceso(datos:ProcesoReeducacionCreate, db:Session = Depends(get_db), usuario_id: int = Depends(obtener_usuario_actual)):
    
    nuevo_proceso=ProcesoReeducacion(**datos.model_dump())
    nuevo_proceso.created_by=usuario_id.id
...
def actualizar_proceso(grupo_id: int, datos: ProcesoReeducacionCreate, db: Session = Depends(get_db), usuario_id: int = Depends(obtener_usuario_actual)):
    ...
    proceso_db.updated_by = usuario_id.id
...
def borrar_proceso(folio: int, db: Session = Depends(get_db), usuario_id: int = Depends(requiere_admin)):
    ...
    if not proceso_db.is_deleted:
        raise HTTPException(status_code=400, detail="Proceso eliminado")

    proceso_db.is_deleted = True
    
    proceso_db.deleted_by = usuario_id.id
