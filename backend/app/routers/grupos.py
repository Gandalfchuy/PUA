...
def crear_grupo(datos: GrupoCreate, db: Session = Depends(get_db), usuario_actual: Usuarios = Depends(obtener_usuario_actual)): 
    ...
    nuevo_grupo.created_by = usuario_actual.id
...
@router_grupo.get("/{grupo_id}", response_model=GrupoResponse)
def obtener_grupo(grupo_id: int, db: Session = Depends(get_db), usuario_actual: Usuarios = Depends(obtener_usuario_actual)):
...
def actualizar_grupo(grupo_id: int, datos: GrupoCreate, db: Session = Depends(get_db), usuario_actual: Usuarios = Depends(obtener_usuario_actual)):
    ...
    datos_completos = datos.model_dump(exclude={"ubicacion"})

    for llave, valor in datos_completos.items():
        setattr(grupo_db, llave, valor)

    grupo_db.updated_by = usuario_actual.id
...
def borrar_grupo(folio: int, db: Session = Depends(get_db), usuario_admin: Usuarios = Depends(requiere_admin)):
    ...
    if grupo_db.is_deleted:
        raise HTTPException(status_code=400, detail="El grupo ya ha sido eliminado previamente")

    grupo_db.is_deleted = True
    grupo_db.deleted_by = usuario_admin.id
